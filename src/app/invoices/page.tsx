...
export default function InvoicesPage() {
    ...
    if (apiInvoices && apiInvoices.length > 0) {
      const mapped: Invoice[] = apiInvoices.map((inv: InvoiceRecord) => {
        ...
        const rawItems = (((inv.workOrder?.items && inv.workOrder.items.length > 0)
          ? inv.workOrder.items
          : (inv.items || [])) as unknown as Array<Record<string, unknown>>)
        ...
        return {
          ...
          customerType: (inv.customer?.type === 'CORPORATE' || inv.customer?.type === 'corporate') ? 'corporate' : 'individual',
          ...
          items: rawItems.length > 0
            ? rawItems.map((i, idx: number) => ({
                id: (i.id as string) || `item_${idx}`,
                type: (i.itemType === 'PART' || i.type === 'PART' ? 'PART' : 'SERVICE') as "SERVICE" | "PART",
                name: (i.name as string) || (i.description as string) || 'Hizmet / Kalem',
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      {/* Page Header */}
      <div data-tour="invs-header" className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
      ...
      {/* KPI Cards Grid */}
      <div data-tour="invs-stats" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div data-tour="invs-ribbon" className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs flex flex-wrap items-center justify-between gap-4 text-xs">
      ...
      {/* Invoice Table */}
      <div data-tour="invs-table" className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs overflow-hidden">

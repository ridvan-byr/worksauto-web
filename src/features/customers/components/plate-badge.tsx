...
export function PlateBadge({ plate, size = "md", className }: PlateBadgeProps) {
  // Format plate uppercase: e.g. "34 RB 1905"
  const formatted = plate.toUpperCase().trim()

  return (
    <div
      className={cn(
        "inline-flex items-center rounded-lg border-2 border-slate-900 dark:border-slate-300 bg-white text-slate-950 font-mono font-black tracking-wider shadow-xs overflow-hidden select-none",
    ...
    >
      ...
      {/* Plate Letters */}
      <span className="pr-1 tracking-widest text-slate-900 font-bold">{formatted}</span>
    </div>
  )
}

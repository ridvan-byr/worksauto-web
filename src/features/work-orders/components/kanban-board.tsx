...
import { Clock, CheckCircle2, Wrench, XCircle, ChevronDown, ChevronUp } from "lucide-react"
...
export function KanbanBoard({ orders, onStatusChange }: KanbanBoardProps) {
  ...
  // 4. CANCELLED: İptal edilen işler (pano dışına düşüp kaybolmaması için)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* CANCELLED ORDERS ACCORDION / TOGGLE */}
      {cancelledOrders.length > 0 && (
        <div className="rounded-2xl border border-rose-200/80 dark:border-rose-900/40 bg-rose-500/[0.03] overflow-hidden transition-all">
          <button
            type="button"
            onClick={() => setShowCancelled(!showCancelled)}
            className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-rose-500/[0.05] transition-colors cursor-pointer"
          >
            <div className="flex items-center gap-2">
              <XCircle size={16} className="text-rose-500" />
              <span className="text-xs font-bold text-rose-700 dark:text-rose-400">
                İptal Edilen İş Emirleri ({cancelledOrders.length})
              </span>
              <span className="text-[11px] text-slate-400">
                (Panoda gizlenen iptal kayıtları)
              </span>
            </div>
            <div className="flex items-center gap-1 text-xs text-rose-600 dark:text-rose-400 font-medium">
              <span>{showCancelled ? "Gizle" : "Göster"}</span>
              {showCancelled ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </div>
          </button>

          {showCancelled && (
            <div className="p-4 pt-0 border-t border-rose-100 dark:border-rose-900/30 grid grid-cols-1 md:grid-cols-3 gap-4 mt-3 animate-in fade-in">

...
export function WorkOrderListView({ orders }: WorkOrderListViewProps) {
  ...
  const filtered = React.useMemo(() => {
    return orders.filter((o) => {
      ...
      const matchPlate = o.plate.toLowerCase().replace(/\s/g, "").includes(q.replace(/\s/g, ""))
      const matchNumber = o.workOrderNumber.toLowerCase().includes(q)
      const matchCustomer = o.customerName.toLowerCase().includes(q)
      return matchPlate || matchNumber || matchCustomer
    })
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
          {[
            ...
            { id: "IN_PROGRESS", label: "Liftte / İşlemde" },
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                <th className="py-3.5 px-4 sm:px-6">İş Emri No</th>
                <th className="py-3.5 px-4">Araç & Plaka</th>
                <th className="py-3.5 px-4">Müşteri</th>
                <th className="py-3.5 px-4">Atanan Lift & Usta</th>
                ...
                filtered.map((order) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      ...
                      <p className="text-[10px] text-slate-400">{order.assignedMechanicName}</p>
                    </td>
